	...
	<div class="djev_items djev_clearfix <?php echo $this->list_class ?>">
		<?php foreach ($this->list as $item) { ?>
			...
			<div class="djev_item djev_clearfix <?php echo $item->published == 1 ? '':'djev_notpublished'; ?> <?php echo $item->featured == 1 ? 'djev_featured':'' ?>">
				<?php if($this->params->get('list_event_poster',1) && !empty($item->thumb)) { ?>
				...
				<div class="djev_item_content">
					...
					<?php if($this->params->get('list_event_title',1)) { ?>
					...
					<?php if($this->params->get('list_event_time',1)) { ?>
					<h4 class="djev_time">
						<?php 
					...
					<?php if($this->params->get('list_event_intro',1)) { ?>
					...
					<div class="djev_info">
						...
						<?php if($this->params->get('list_event_category',1)) { 
						...
						<?php if($this->params->get('list_event_readmore',1)) { ?>
