...
class NTmhOAuth {
    ...
    private function reset_request_settings($options = array()) {
        ...
        if (!empty($options)) $this->request_settings = array_merge($this->request_settings, $options);
    }
    ...
    private function prepare_oauth1_params() {
        ...
        if ($oauth_token = $this->token()) $defaults['oauth_token'] = $oauth_token;
    ...
    private function token() {
        if ($this->request_settings['with_user']) {
            if (isset($this->config['token']) && !empty($this->config['token'])) return $this->config['token']; elseif (isset($this->config['user_token'])) return $this->config['user_token'];
        }
    ...
    private function secret() {
        if ($this->request_settings['with_user']) {
            if (isset($this->config['secret']) && !empty($this->config['secret'])) return $this->config['secret']; elseif (isset($this->config['user_secret'])) return $this->config['user_secret'];
        }
    ...
    private function multipart_escape($value) {
        if (!$this->request_settings['multipart'] || strpos($value, '@') !== 0) return $value;
    ...
    private function prepare_params() {
        ...
        foreach ($params as $k => $v) {
            ...
            if ($doing_oauth1) {
                ...
                if ((strpos($k, 'oauth') === 0) || !$this->request_settings['multipart']) $prepared_pairs_with_oauth[] = "{$k}={$v}";
            ...
            if (!$this->request_settings['multipart']) $prepared_pairs[] = "{$k}={$v}";
        }
    ...
    private function prepare_oauth_signature() {
        $this->request_settings['oauth1_params']['oauth_signature'] = $this->safe_encode(n2_base64_encode(hash_hmac('sha1', $this->request_settings['basestring'], $this->request_settings['signing_key'], true)));
    }
    ...
    private function prepare_auth_header() {
        ...
        if (isset($header)) $this->request_settings['headers']['Authorization'] = $header;
    }
    ...
    public function bearer_token_credentials() {
        ...
        return n2_base64_encode($credentials);
    }
    ...
    private function update_metrics() {
        $now = time();
        if (($this->metrics['interval_start'] + $this->config['streaming_metrics_interval']) > $now) return null;
    ...
    public function url($request, $extension = 'json') {
        ...
        if (substr($request, $pos) === $extension) $request = substr_replace($request, '', $pos);
    ...
    private function curlWrite($ch, $data) {
        ...
        if (!is_callable($this->config['streaming_callback'])) return 0;
    ...
    private function curlit() {
        ...
        if ($this->request_settings['method'] == 'GET' && isset($this->request_settings['querystring'])) {
            ...
            $postfields = array();
            if (isset($this->request_settings['postfields'])) $postfields = $this->request_settings['postfields'];
        ...
        if ($this->config['curl_proxyuserpwd'] !== false) curl_setopt($c, CURLOPT_PROXYUSERPWD, $this->config['curl_proxyuserpwd']);

        if ($this->config['curl_sslversion'] !== false) curl_setopt($c, CURLOPT_SSLVERSION, $this->config['curl_sslversion']);
        ...
        if (isset($this->config['block']) && (true === $this->config['block'])) return 0;
